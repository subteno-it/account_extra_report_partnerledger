...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _sum_partner(self, data, partner, field):
        if field not in ['debit', 'credit', 'debit - credit']:
            return
        result = 0.0
        query_get_data = self.env['account.move.line'].with_context(data['form'].get('used_context', {}), date_from=False)._query_get()
    ...
    def _initial_balance_partner(self, data, partner, field):
        if field not in ['debit', 'credit', 'debit - credit']:
            return
        result = 0.0
    ...
    def render_html(self, data):
        ...
        if data['form']['account_exclude_ids']:
            data['computed']['account_ids'] = list(set(data['computed']['account_ids']) - set(data['form']['account_exclude_ids']))
        ...
        self.env.cr.execute(query, tuple(params))
        partner_move_ids = [res['partner_id'] for res in self.env.cr.dictfetchall()]
        if data['form'].get('partner_ids'):
            ...
            self.env.cr.execute(query, tuple(params))
            partner_initial_move_ids = [res['partner_id'] for res in self.env.cr.dictfetchall() if float(res['sum'] or 0.) != 0.]
        ...
        docargs = {
            ...
            'initial_balance_partner': self._initial_balance_partner,
        }
